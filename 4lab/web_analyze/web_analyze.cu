...
// CUDA Kernel для детектирования уязвимостей (оптимизированная версия)
__global__ void detectVulnerabilities(const char* inputs, int* results, int n, int input_len) {
    ...
    if (vuln == VULN_NONE && (d_strstr(str, "../", len) || d_strstr(str, "..\\", len) ||
        d_strstr(str, "%2e%2e%2f", len) || d_strstr(str, "%2e%2e/", len) ||
        d_strstr(str, "..%2f", len) || d_strstr(str, "/etc/passwd", len) ||
        d_strstr(str, "/etc/shadow", len) || d_stristr(str, "c:\\windows", len))) {
        vuln = VULN_PATH_TRAVERSAL;
    }

    // === XSS (Cross-Site Scripting) ===
    if (vuln == VULN_NONE && (d_stristr(str, "<script", len) || d_stristr(str, "javascript:", len) ||
        d_stristr(str, "onerror=", len) || d_stristr(str, "onload=", len) ||
        d_stristr(str, "onclick=", len) || d_stristr(str, "onmouseover=", len) ||
        d_stristr(str, "<iframe", len) || d_stristr(str, "<svg", len) ||
        d_stristr(str, "alert(", len) || d_stristr(str, "document.cookie", len) ||
        d_stristr(str, "eval(", len) || d_strstr(str, "%3cscript", len))) {
        vuln = VULN_XSS;
    }

    // === SQL Injection ===
    if (vuln == VULN_NONE && (d_stristr(str, "' or '", len) || d_stristr(str, "' or 1=1", len) ||
        d_stristr(str, "\" or \"", len) || d_stristr(str, "1=1--", len) ||
        d_stristr(str, "' --", len) || d_stristr(str, "'; drop", len) ||
        d_stristr(str, "union select", len) || d_stristr(str, "union all select", len) ||
        d_stristr(str, "select * from", len) || d_stristr(str, "insert into", len) ||
        d_stristr(str, "delete from", len) || d_stristr(str, "drop table", len) ||
        d_stristr(str, "exec(", len) || d_stristr(str, "execute(", len) ||
        d_stristr(str, "waitfor delay", len) || d_stristr(str, "benchmark(", len) ||
        d_stristr(str, "sleep(", len) || d_stristr(str, "having 1=1", len))) {
        vuln = VULN_SQL_INJECTION;
    }

    // === Command Injection ===
    if (vuln == VULN_NONE && (d_strstr(str, "; ls", len) || d_strstr(str, "| ls", len) ||
        d_strstr(str, "& ls", len) || d_strstr(str, "; cat", len) ||
        d_strstr(str, "| cat", len) || d_strstr(str, "; rm", len) ||
        d_strstr(str, "; wget", len) || d_strstr(str, "; curl", len) ||
        d_strstr(str, "| nc ", len) || d_strstr(str, "`", len) ||
        d_strstr(str, "$(", len) || d_strstr(str, "%0a", len) ||
        d_strstr(str, "; ping", len) || d_strstr(str, "; whoami", len) ||
        d_strstr(str, "| whoami", len) || d_strstr(str, "/bin/sh", len) ||
        d_strstr(str, "/bin/bash", len) || d_stristr(str, "cmd.exe", len) ||
        d_stristr(str, "powershell", len))) {
    ...
    if (vuln == VULN_NONE && (d_stristr(str, "<!entity", len) || d_stristr(str, "<!doctype", len) ||
        d_strstr(str, "system \"file:", len) || d_strstr(str, "system 'file:", len) ||
        d_strstr(str, "expect://", len))) {
        vuln = VULN_XXE;
    }

    // === SSRF (Server-Side Request Forgery) ===
    if (vuln == VULN_NONE && (d_strstr(str, "127.0.0.1", len) || d_strstr(str, "localhost", len) ||
        d_strstr(str, "0.0.0.0", len) || d_strstr(str, "169.254.", len) ||
        d_strstr(str, "10.0.", len) || d_strstr(str, "192.168.", len) ||
        d_strstr(str, "172.16.", len) || d_strstr(str, "file://", len) ||
        d_strstr(str, "gopher://", len) || d_strstr(str, "dict://", len))) {
        vuln = VULN_SSRF;
    }
...
int main() {
    ...
    printf("  Results\n");
    printf("==============================================\n");
    printf("Total: %d | Correct: %d | Errors: %d\n", n, correct, n - correct);
    ...
    printf("F1 >= 96%%:        %s\n", f1 >= 0.96 ? "PASS" : "FAIL");
    printf("Throughput >= 10k: %s\n", throughput >= 10000 ? "PASS" : "FAIL");

    // Тест производительности
    printf("\n=== Performance Test (100000 strings) ===\n");

    int perf_n = 100000;
    char* perf_h = (char*)malloc(perf_n * MAX_INPUT_LEN);
    int* perf_r = (int*)malloc(perf_n * sizeof(int));
    memset(perf_h, 0, perf_n * MAX_INPUT_LEN);

    for (int i = 0; i < perf_n; i++) {
        strncpy(&perf_h[i * MAX_INPUT_LEN], testCases[i % numTestCases].input, MAX_INPUT_LEN - 1);
    }

    char* perf_d_in;
    int* perf_d_out;
    cudaMalloc(&perf_d_in, perf_n * MAX_INPUT_LEN);
    cudaMalloc(&perf_d_out, perf_n * sizeof(int));
    cudaMemcpy(perf_d_in, perf_h, perf_n * MAX_INPUT_LEN, cudaMemcpyHostToDevice);
